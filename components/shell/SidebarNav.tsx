...
interface Props {
  collapsed: boolean;
  /** Chamado ao clicar num link — a drawer mobile usa isto pra se fechar sozinha. */
  onNavigate?: () => void;
  /** Falso na drawer mobile: lá "colapsar" não existe, fechar é a própria ação de sair. */
  showCollapseToggle?: boolean;
}

/**
 * O conteúdo da navegação principal — extraído de `Sidebar.tsx` (issue: sem
 * padrão mobile) pra ser desenhado tanto pelo `<aside>` fixo do desktop quanto
 * pela drawer (`MobileNav`) sem duplicar a lista de grupos/itens.
 *
 * Não decide nada: `sidebarGroups()` (lib/navigation/registry.ts) resolve
 * quais grupos e destinos este papel vê.
 */
export function SidebarNav({ collapsed, onNavigate, showCollapseToggle = true }: Props) {
  ...
  return (
    <>
      ...
      <nav className="flex-1 space-y-3 overflow-y-auto overflow-x-hidden [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] p-2" aria-label="Navegação principal">
