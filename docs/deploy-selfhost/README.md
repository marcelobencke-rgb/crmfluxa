# Fluxa CRM self-hosted — instalação em VPS (com agente de IA)
...
```bash
git clone https://github.com/fluxacrm/fluxacrm.git && cd fluxacrm
...
- **Backup diário** (do seu crontab na VPS):
  `0 3 * * * /caminho/repo/scripts/backup-db.sh /var/backups/fluxa`
