...
| **Agendamento + Catálogo** (spec 18, PR #11, mesclado 2026-08-17) | Schema+API+UI+tools MCP prontos, verificados manualmente pela tela (não é auditoria — é relato direto de quem construiu). Vínculo agendamento↔lead provado de ponta a ponta (timeline grava `appointment_scheduled`). `invariants` do CI (`pnpm test:db` num runner de verdade) **passou** contra as migrations 0146/0147 — isolamento RLS das 9 tabelas novas está provado, diferente do que a versão anterior desta linha dizia. Também corrigidas nesta PR: 3 tools de agendamento sem classificação em `ALVO_DE_FUNIL`/`ESCRITA_QUE_E_TRABALHO_DE_ATENDENTE`, estouro do teto de 20 tools/agente (bundle "Agendar horários" criado à parte de "Atender e responder"), colisão de nome `resourceId` no scanner de audit | **`e2e` foi mesclado VERMELHO, com aval do dono** — dois achados novos, nenhum de RLS/dado: (1) `navegacao.spec.ts` "nenhum grupo fica fora da dobra… 900px" — os 3 itens de nav novos (Catálogo/Recursos/Agendamentos) estouram o menu na viewport estreita, precisa de trabalho de layout responsivo ainda não feito; (2) dois timeouts de visibilidade em `escopo-de-funil-do-agente.spec.ts:94` e `agente-papeis-operador.spec.ts:147`, sem ligação de código encontrada com esta PR — hipótese não comprovada é que a página `/app/ai/agents/[id]` (já documentada no próprio spec como lenta, >30s no primeiro load) ficou mais pesada com o catálogo indo de 51 para 57 tools. `e2e` já falhava de forma independente na `main` antes desta PR (mesmas ~13 falhas em 4 execuções seguidas) — não é regressão de bloqueio, é dívida que soma. Zero `tests/e2e/*.spec.ts` e zero `tests/invariants/*.test.ts` NOVOS escritos especificamente pra agendamento (a suíte existente só provou que nada quebrou). Sync Google Agenda sem credencial real testada (só degradação sem env verificada). Criação automática do canal de push (watch) não implementada — pull só via poll de 15min. Sem CRUD de `crm_resource_time_off` (férias/feriado) apesar da tabela existir. Tools MCP nunca chamadas pelo agente de verdade (só via REST equivalente). Vocabulário de agendamento e pacote de múltiplos serviços num horário só, fora de escopo por decisão do dono |
