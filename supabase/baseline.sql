...
-- ---- catálogo (produtos/serviços) + agendamento + sync Google Agenda (migration 0146) ----
--
-- 7 tabelas novas (crm_products, crm_resources, crm_resource_services,
-- crm_resource_availability, crm_resource_time_off, crm_calendar_connections,
-- crm_appointments) — spec em docs/specs/18-spec-agendamento-catalogo.md. Todas RLS
-- tenant-aware exceto crm_calendar_connections (admin-only, guarda credencial cifrada).
-- Reaproveita fn_encrypt_oauth/fn_decrypt_oauth (migration 0006) pra token do Google — sem
-- cifra nova, sem env var nova de chave. crm_lead_links já reservava 'appointment' em
-- target_kind desde a migration 0003; esta é a primeira migration a usar.
...
-- ---- suporte que faltava pro worker de pull do Google Agenda (migration 0147) ----
--
-- watch_resource_id: o Google atribui um resourceId ao canal de watch; channels.stop
-- exige ele junto do id que nós geramos, senão disconnect não encerra o canal lá.
--
-- fn_crm_pull_upsert_appointment: o "SET LOCAL fluxa.sync_origin" que evita eco
-- Google->Fluxa->Google só é alcançável dentro de uma RPC (1 chamada = 1 transação);
-- do cliente Supabase JS não tem canal pra rodar SET LOCAL antes de um .update() na
-- MESMA transação.
